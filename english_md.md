...
\* \*\*Environment Dependency\*\*: Requires `GM\\\_setValue` and other advanced APIs. Recommended for use with \*\*Tampermonkey\*\* or \*\*ScriptCat\*\*.
...
The script communicates with Bilibili's React/Vue framework by intercepting and simulating `StorageEvent`. By locking the `theme\\\_style` field in `Cookie` and `LocalStorage` during the `document-start` phase, it achieves a smarter, more integrated automation than the official toggle.
